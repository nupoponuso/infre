		...
		_Out_opt_ TexMetadata* pTexInfo)
	{
		*textureView = nullptr;
		...
		if (wicDataSize >= 18 && memcmp(&wicData[wicDataSize - 18], "TRUEVISION-XFILE.", 18) == 0) {
			...
			hr = LoadFromDDSMemory(wicData, wicDataSize, (DDS_FLAGS)0, &meta, image);
		}
		else {
			hr = LoadFromWICMemory(wicData, wicDataSize, (WIC_FLAGS)0, &meta, image);
		}
		...
		_Out_opt_ TexMetadata* pTexInfo)
	{
		*textureView = nullptr;
		...
		if (_wcsicmp(wszExt, L".tga") == 0)
			...
			hr = LoadFromDDSFile(szFileName, (DDS_FLAGS)0, &meta, image);
		else
			hr = LoadFromWICFile(szFileName, (WIC_FLAGS)0, &meta, image);
		if (FAILED(hr)) return hr;
		if (pTexInfo) *pTexInfo = meta;
		return CreateShaderResourceView(d3dDevice, image.GetImages(), image.GetImageCount(), meta, textureView);
	}
